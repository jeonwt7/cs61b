...
public class GraphDB {
     ...
     */
    Map<Long, HashSet<Long>> graph = new LinkedHashMap<>();
    Map<Long, Vertex> idVertex = new LinkedHashMap<>();
    KdTree tree;
    ...
    private void clean() {
        ...
        this.tree = new KdTree(idVertex); // initialize KdTree
    }
     ...
     */
    double lon(long v) {
        if (idVertex.keySet().contains(v)) {
            return idVertex.get(v).lon;
        } else {
            return 0;
        }
    }
    ...
    public long closest(double lon, double lat) {
        Vertex v = new Vertex(0, lon, lat);
        long toReturn = 0;
        double minDist = Double.MAX_VALUE;
        idVertex.put((long) 0, v);
        for (long id:idVertex.keySet()) {
            if (id != 0 && distance(id, 0) < minDist) {
                minDist = distance(id, 0);
                toReturn = id;
            }
        }
        idVertex.remove((long) 0);
        return toReturn;
    }
    */

    public long closest(double lon, double lat) {
        return tree.nearest(projectToX(lon, lat), projectToY(lon, lat));
    }
